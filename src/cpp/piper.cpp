...
#define STR(x) _STR(x)
  ...
  Phoneme getCodepoint(std::string s) {
    if (s.length() <= 3 && std::all_of(s.begin(), s.end(), [](char c) { return static_cast<unsigned char>(c) < 128; })) {
      Phoneme result = 0;
      for (char c : s) result = (result << 8) | static_cast<unsigned char>(c);
      return result;
    }
  ...
  void parsePhonemizeConfig(json& configRoot, PhonemizeConfig& phonemizeConfig) {
    ...
    if (configRoot.contains("phoneme_type")) {
      ...
      } else if (phonemeTypeStr == "phonemes") {
  ...
  void textToAudio(PiperConfig& config, Voice& voice, std::string text,
    ...
    if (config.phonemeInput || voice.phonemizeConfig.phonemeType == RawPhonemes) {
      parse_phonemes(text, phonemes);
  ...
  void parse_phonemes(std::string text, std::vector<std::vector<Phoneme>>& phonemes) {
    bool has_hyphen = text.find('-') != std::string::npos;
    ...
    while (iss >> word) {
      ...
      if (has_hyphen) {
        for (size_t start = 0, end; start < word.size(); start = end + 1) {
          end = word.find('-', start);
          if (end == std::string::npos) {
            end = word.size();
          }
          if (end > start) {
            word_phonemes.push_back(getCodepoint(word.substr(start, end - start)));
          }
          if (end == word.size()) {
            break;
          }
        ...
        for (char c : word) {
          word_phonemes.push_back(static_cast<Phoneme>(c));
        }
      }

      if (!word_phonemes.empty()) {
        phonemes.push_back(word_phonemes);
      }
    }
  }
  ...
  void phonemes_to_ids(const std::vector<Phoneme>& phonemes, PhonemeIdConfig& config,
    ...
    if (config.phonemeIdMap) {
      ...
      phonemeIdMap = std::make_shared<PhonemeIdMap>(DEFAULT_PHONEME_ID_MAP);
    }
    auto add_phoneme_ids = [&phonemeIds, &phonemeIdMap, &missingPhonemes](Phoneme phoneme) {
      if (phonemeIdMap->count(phoneme) > 0) {
        auto const ids = &(phonemeIdMap->at(phoneme));
        phonemeIds.insert(phonemeIds.end(), ids->begin(), ids->end());
      } else {
    ...
    if (config.addBos) {
      add_phoneme_ids(config.bos); // Beginning of sentence symbol (^)
      if (config.interspersePad) {
        add_phoneme_ids(config.pad); // Pad after bos (_)
      }
    }
    // Add ids for each phoneme 
    for (auto const phoneme : phonemes) {
      add_phoneme_ids(phoneme);
      if (config.interspersePad) {
        add_phoneme_ids(config.pad); // pad (_)
      }
    }
    if (config.addEos) {
      add_phoneme_ids(config.eos); // End of sentence symbol ($)
    }
  }
