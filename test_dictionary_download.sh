...
if echo "テスト" | "$PIPER_BIN" --model "$PIPER_DIR/test/models/ja_JP-test-medium.onnx" --output_file test1.wav 2>&1 | grep -q "Please download the dictionary manually"; then
...
if echo "こんにちは" | "$PIPER_BIN" --model "$PIPER_DIR/test/models/ja_JP-test-medium.onnx" --output_file test2.wav; then
...
if echo "ありがとう" | "$PIPER_BIN" --model "$PIPER_DIR/test/models/ja_JP-test-medium.onnx" --output_file test3.wav 2>&1 | grep -q "Downloading"; then
...
if echo "さようなら" | "$PIPER_BIN" --model "$PIPER_DIR/test/models/ja_JP-test-medium.onnx" --output_file test4.wav; then
